...
def authenticate():
    return ('Access denied.\n'
            'Please use proper credentials', 401,
...
def create_bar(training_data, top_n=20):
    try:
        ...
        app.logger.error(f"Error in create_bar: {e}")
...
def create_scatter_plot(training_data):
    try:
        ...
        text = [clean_string(word) for word in numeric_data['Word'].fillna('').tolist()]
...
def index():
    try:
        ...
        if request.method == 'POST':
            ...
            app.logger.info(f"{datetime.now()} - Classified {len(files)} emails.")

        #Create the three visualizations
        charts['word_frequency'] = create_bar(training_data)
